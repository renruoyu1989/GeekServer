...
namespace BeetleX.Proxy
{
    ...
    /// </summary>
    public abstract class XDispatchProxy
    {

        public static bool DynamicAssemblyCollectible { get; set; } = true;

        protected XDispatchProxy()
        {
        }
        ...
        public static T Create<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] T, 
            [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicParameterlessConstructor)] TProxy>()
            where TProxy : XDispatchProxy
        {
            return (T)XDispatchProxyGenerator.CreateProxyInstance(typeof(TProxy), typeof(T));
        }
    }
}
